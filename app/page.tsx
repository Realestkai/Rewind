  ...
  return <main id="main-content">
    ...
    <header className="site-header">
      ...
      <nav className={menuOpen ? "nav open" : "nav"} aria-label="Main navigation">
        <Link href="/store" onClick={() => setMenuOpen(false)}>Store</Link><Link href="/commissions">Commissions</Link><Link href="/accessibility">Accessibility</Link><Link href="/dashboard">Dashboard</Link>
      </nav>
      <div className="header-actions"><Link className="quiet-link" href="/profile">{user ? user.username : "Profile"}</Link>{user ? <Link className="button button-small" href="/dashboard">Dashboard</Link> : <Link className="button button-small" href="/login">Sign in with Discord</Link>}<button className="menu-button" aria-label={menuOpen ? "Close menu" : "Open menu"} onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X size={20} /> : <Menu size={20} />}</button></div>
    </header>
